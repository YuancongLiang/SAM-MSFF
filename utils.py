...
def mixup_data(x, y, alpha=1.0):
    lam = torch.tensor([torch.rand(1) for _ in range(x.size(0))])  # 随机生成混合系数 lamda
    lam = torch.max(lam, 1 - lam)  # 取混合系数和 1 - 混合系数的最大值
    batch_size = x.size(0)

    index = torch.randperm(batch_size)  # 随机生成索引组合

    mixed_x = lam.view(batch_size, 1, 1, 1) * x + (1 - lam).view(batch_size, 1, 1, 1) * x[index, :]
    mixed_y = lam.view(batch_size, 1, 1) * y + (1 - lam).view(batch_size, 1, 1) * y[index, :]
