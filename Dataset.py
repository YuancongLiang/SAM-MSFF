...
class Chasedb1Dataset(Dataset):
    def __init__(self, data_dir=r"./chasedb1/chasedb1", image_size=256, mode='train', requires_name=True, point_num=1, mask_num=2):
        """
        Initializes a training dataset.
        Args:
            data_dir (str): Directory containing the dataset.
            ...
            requires_name (bool, optional): Indicates whether to include image names in the output. Defaults to True.
            num_points (int, optional): Number of points to sample. Defaults to 1.
            num_masks (int, optional): Number of masks to sample. Defaults to 2.
        """
        ...
        self.point_num = point_num
        self.mask_num = mask_num
        self.pixel_mean = [7.11364493, 41.80026049, 115.48434409]
        self.pixel_std = [8.96785366, 35.76312054, 86.67236085]

        self.image_names = []
        for i in range(1, 10):
            self.image_names.append("Image_" + '0' + str(i) + "R")
            self.image_names.append("Image_" + '0' + str(i) + "L")
        for i in range(10, 15):
            self.image_names.append("Image_" + str(i) + "R")
            self.image_names.append("Image_" + str(i) + "L")
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join(self.data_dir, self.image_names[index] + ".jpg")
        mask1st_path = os.path.join(self.data_dir, self.image_names[index] + "_1stHO.png")
        mask2nd_path = os.path.join(self.data_dir, self.image_names[index] + "_2ndHO.png")
        ...
        mask_path = [mask1st_path, mask2nd_path]
        for m in mask_path:
            pre_mask = cv2.imread(m, 0)
            if pre_mask.max() == 255:
                pre_mask = pre_mask / 255
    ...
    def get_mean_std(self):
        ...
        for image_name in tqdm(self.image_names):
            image_path = os.path.join(self.data_dir, image_name + ".jpg")
            image = cv2.imread(image_path)
            for d in range(3):
                cumulative_mean[d] += image[:, :, d].mean()
                cumulative_std[d] += image[:, :, d].std()
        self.pixel_mean = cumulative_mean / len(self.image_names)
        self.pixel_std = cumulative_std / len(self.image_names)
...
def collate_fn(batch):
    return stack_dict_batched(batch)
if __name__ == "__main__":
    ...
    train_dataset = Chasedb1Dataset("./chasedb1/chasedb1", image_size=256, mode='train', requires_name=True, point_num=1,
                                    mask_num=2)
